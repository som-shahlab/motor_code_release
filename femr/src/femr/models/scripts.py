...
def train_model() -> None:
    ...
    parser.add_argument("--motor_dim", type=int)
    ...
    if task["type"] == "motor":
        ...
        task["dim"] = args.motor_dim
    elif task["type"] == "next_code":
    ...
    if args.start_from_checkpoint is not None:
        with open(os.path.join(args.start_from_checkpoint, "best"), "rb") as f:
            ...
            if (
                ...
                and ("EHRTransformer/~/MOTORTask/~/linear" in checkpointed_weights)
            ):
                magic_layer = checkpointed_weights["EHRTransformer/~/MOTORTask/~/linear"]
