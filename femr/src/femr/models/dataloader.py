...
def create_batches() -> None:
    ...
    parser.add_argument("--task", type=str, help="Either next_code, motor, or labeled_patients")
    parser.add_argument("--transformer_vocab_size", type=int, default=1024 * 64, help="Size of the transformer vocab")
    parser.add_argument(
        "--survival_dictionary_path", type=str, help="The survival dictionary if running that task"
    )
    ...
    if args.labeled_patients_path is not None:
        ...
        with open(args.survival_dictionary_path, "rb") as f:
            surv_dict = msgpack.load(f, use_list=False)
        task = {
            "type": "motor",
            "survival_dict": surv_dict,
        }
    elif args.task == "next_code":
        task = {"type": "next_code", "vocab_size": 1024 * 8}
    else:
