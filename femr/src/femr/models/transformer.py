...
class SurvivalTask(hk.Module):
    def __init__(self, config):
        ...
        Please read the MOTOR paper to understand the algorithm here.
        """
...
class NextCodeTask(hk.Module):
    def __init__(self, config):
        super().__init__(name="NextCodeTask")
        self.config = config
...
class MOTORTask(hk.Module):
    def __init__(self, config, get_time_reprs=False):
        """Please read the MOTOR paper to understand the algorithm here."""
        super().__init__(name="MOTORTask")
        self.config = config
...
def create_task(config):
    if config["type"] == "next_code":
        return NextCodeTask(config)
    elif config["type"] == "motor":
        return MOTORTask(config)
