...
parser.add_argument("--survival_batches", type=str, default=None)
...
with open(
    os.path.join(args.survival_batches, "batch_info.msgpack"),
    "rb",
) as f:
...
assert label_pids.dtype == np.int64
...
for frac in [1]:
    ...
    for l_exp in np.linspace(end_l, start_l, num=20):
        ...
        def get_c(split_index):
            ...
            limit_time = jnp.quantile(event_times[~is_censors], 0.9)
    ...
    print("Debug info", limit_time, np.sum(mask), np.mean(is_censor), np.mean(event_time))
    ...
    for i in range(hazards.shape[0]):
        ...
        assert event_time == (target_label.value.time_to_event) / datetime.timedelta(minutes=1), f"{event_time} {target_label.value.time_to_event} {target_labels} {pid} {age}"
