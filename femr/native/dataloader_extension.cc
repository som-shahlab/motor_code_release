...
class NextCodeTask : public Task {
   public:
    NextCodeTask(json config, PatientDatabase& data) : Task(config, data) {
...
class MOTORTask : public Task {
   public:
    MOTORTask(json config, PatientDatabase& data, Ontology& ontology)
        : Task(config, data) {
        // Might be empty, in which case we train on everyone
...
std::unique_ptr<Task> create_task(json config, PatientDatabase& data,
    ...
    } else if (type == "next_code") {
        return std::make_unique<NextCodeTask>(config, data);
    } else if (type == "motor") {
        return std::make_unique<MOTORTask>(config, data, ontology);
    }
